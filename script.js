...
// ============================================================
// PRÉCHARGEMENT (assets-mobile pour mobile, assets pour desktop)
// ============================================================

// Sur mobile (≤600px) on utilise le dossier assets-mobile/ avec ses images
// au cadrage portrait spécifique. Sur desktop on garde assets/ classique.
const isMobile = window.matchMedia('(max-width: 600px)').matches;
const folder = isMobile ? 'assets-mobile' : 'assets';
const suffix = isMobile ? '-md' : '';

const IMAGES_TO_LOAD = [
    `${folder}/aqueduc${suffix}.webp`,
    `${folder}/mur-fond${suffix}.webp`,
    `${folder}/maison${suffix}.webp`,
    `${folder}/statue${suffix}.webp`,
    `${folder}/collines-devant${suffix}.webp`,
    `${folder}/plantes${suffix}.webp`,
    `${folder}/blees-devant${suffix}.webp`
];
...
// Sur mobile portrait, on agrandit le premier plan pour que les plantes
// des bords latéraux soient visibles malgré le cadrage portrait
// Plus besoin du boost mobile : les images du dossier assets-mobile/
// sont déjà cadrées en portrait avec tout le contenu visible.
const FOREGROUND_BOOST = 1.0;
...
function updateParallax() {
    ...
    const ph1e = eIO(ph1);
    layerCollines.style.transform = `translate3d(0, ${ph1e * 90}%, 0) scale(${(1 + ph1e * 0.4) * FOREGROUND_BOOST})`;
    layerCollines.style.opacity = clamp(1 - eIQ(ph1) * 1.3, 0, 1);

    // ===== PHASE 2 : PLANTES (0.18 → 0.48) — démarre à 60% de phase 1 =====
    const ph2 = clamp((progress - 0.18) / 0.30, 0, 1);
    const ph2e = eIO(ph2);
    layerPlantes.style.transform = `translate3d(0, ${ph2e * 100}%, 0) scale(${(1 + ph2e * 0.7) * FOREGROUND_BOOST})`;
    layerPlantes.style.opacity = clamp(1 - eIQ(ph2) * 1.5, 0, 1);

    // ===== PHASE 3 : BLÉS (0.30 → 0.55) =====
    const ph3 = clamp((progress - 0.30) / 0.25, 0, 1);
    const ph3e = eIO(ph3);
    layerBlees.style.transform = `translate3d(0, ${ph3e * 130}%, 0) scale(${(1 + ph3e * 1.2) * FOREGROUND_BOOST})`;
    ...
    /* ============================================================
       PHASE 5 : LUEUR QUI ÉMERGE (0.65 → 0.85)
       Une lueur dorée jaillit depuis la porte centrale,
       comme si une lumière chaude sortait du bâtiment
       ============================================================ */
    const ph5 = clamp((progress - 0.65) / 0.20, 0, 1);
    const ph5e = eOQ(ph5);

    // La lueur grossit et s'intensifie progressivement
    doorGlow.style.opacity = ph5 * 0.95;
    doorGlow.style.transform = `translate(-50%, -50%) scale(${0.5 + ph5e * 4}) translateZ(0)`;

    /* ============================================================
       PHASE 6 : TRAVERSÉE — on plonge vers la maison (0.80 → 1.0)
       Zoom rapide qui plonge vers la lueur, le décor s'estompe
       ============================================================ */
    const ph6 = clamp((progress - 0.80) / 0.20, 0, 1);
    const ph6e = eIQ(ph6);

    if (ph6 > 0) {
        const dashScale = 1 + ph6e * 4;
        ...
        // Tout le décor s'estompe au fur et à mesure
        ...
        // La lueur s'étend et finit par remplir l'écran
        doorGlow.style.opacity = clamp(0.95 + ph6e * 0.5, 0, 1);
        doorGlow.style.transform = `translate(-50%, -50%) scale(${4.5 + ph6e * 8}) translateZ(0)`;
    } else {
